...
const fixedOverlayStyle: React.CSSProperties = {
  ...overlayStyle,
  position: 'fixed',
  top: '80px',
  zIndex: 10,
...
const Layout: React.FC = () => {
  ...
  const [overlayFixed, setOverlayFixed] = React.useState(false);
  // const [overlayRect, setOverlayRect] = React.useState();

  const handleOverlayRef = React.useRef<HTMLInputElement>(null);

  const overlayRect = React.useMemo(
    () =>
      _.pick(
        handleOverlayRef.current?.getBoundingClientRect(),
        'height',
        'width',
      ),
    [handleOverlayRef],
  ...
  // useEffect(() => {
  //   if (typeof overlayRect !== 'undefined') {
  //     setOverlayRect(() =>
  //       _.pick(
  //         handleOverlayRef.current?.getBoundingClientRect(),
  //         'height',
  ...
  //     );
  //   }
  // }, [overlayRect, handleOverlayRef]);

  const stickOverlay = React.useCallback(() => {
    setOverlayFixed(true);
  }, []);
  const stickTopMenu = React.useCallback(() => {
    setMenuFixed(true);
  }, []);
  const unStickOverlay = React.useCallback(() => {
    setOverlayFixed(false);
  }, []);
